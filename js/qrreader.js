...
/*this is what happens here:
 the user open the web the first time, it will automatically show the camera.
 the user point the camera to the QR code, it will give a string (the company name)
 we use that string to access the database and take the latitude and logitude of the restaurant,
 them we take the latitude and longitude of the user and we compare both to see the user position , 
 if he is less than 100 meters he can continue,
 we send the company name to the php and we open the menu list of that company,
 there the user can start selecting what he want.
 */


//here we receive the array of the restaurant or stablishment and save it to compare it with the user position
function PlacePosition(latitude, longitude, url) {
    aimlat = latitude;
    aimlon = longitude;
    aimurl = url;

}
//this code find the user actual position and call CalculatePosition if sucess
function geoFindMe() {
...
//we store the user position and call Calculate position to compare the user position with the restaurant position
...
function CalculatePosition() {
    ...
    for (var x = 0; x < check_location.length; x += 2) {

        var lat2 = check_location[x];
        var lon2 = check_location[x + 1];
        ...
        var R = 6371; // km
        var dLat = toRad(lat2 - currentlat);
        var dLon = toRad(lon2 - currentlon);
...
//the given position is in kilomenters! so 0.1 is 100 meters, if the user is close we trigger the camera, if not we send a message
        if (d > 0.4) {
            alert("error 302");
        } else {

            $.ajax({
                url: '../client/includes/posts.php',
                data: {'redirect': aimurl},
                type: 'post',
                success: function (data) {


                    window.open("../client/qr_reader.php", "_self");
                },
...
function StartCamera() {
    ...
    // select html element "display"
    var webcam = document.getElementById('display');
// turn on the cam
    ...
    // taking canvas to paint
    var canvas = document.getElementById('qr-canvas');
    var context = canvas.getContext('2d');
    var webcam = document.getElementById('display');

//it try find a qr pattern every second, if we turn it off with a simple if it will not look for more
    window.setInterval(function () {
        //a valid url is stored in result, if its null we don't have a valid url yet so it will keep looking for it
        if (result == null) {
            paint();
        }
    }, 1000);
...
// paint in a canvas the video content
    function paint() {

        context.drawImage(webcam, 0, 0, 640, 480);
        //calling the code function, it will look for qr-canvas
        qrcode.decode();

        //if it find a pattern it give back a string, we send it to "read";
        qrcode.callback = read;
    }
    //we check the QR, if it is a Quiku QR we send the user to the given page by recharging the qr_reader.php (it will now load the iframe instead of the camera)
    function read(a) {

        alert(a);
        var index = a.indexOf("table");
        var end = a.length;
        var db= a.substring(0, index - 1);
        alert("start: "+index+" end: "+end+" result: "+ result);
        ...
        result = "found";
        /*window.location.href = "qr_reader.php?qrurl=" + QRString; 
         stop=false;*/
    }
}

//we call this function to realod the page if everything is correct
function Reload() {
